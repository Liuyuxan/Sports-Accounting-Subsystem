...
<template>
  <div class="personal title-center">
    ...
    <Form ref="formData" :model="formData.value" :label-width="50" style="width: 650px">
        ...
        <template v-for="(item, index) in formRef.infos.value" :key="index">
          <div class="content">
            <FormItem :label="'学号' + Number(index + 1)">
              <Row>
                <Col>
                  <Input v-model="item.id" placeholder="学号" style="width: 275px"/>
                </Col>
                <Col>
      ...
      <div class="submit">
        <Button type="primary" class="btn" @click="submit" style="width: 100px;">提交并导出</Button>
      </div>
...
import useDownload from '@/stores/modules/download';
...
const downloadUid = storeToRefs(downloadStore)
...
const submit = () => {
  ...
  YXrequest.post({
    ...
    // window.open(`http://192.168.116.17:8080//Excel/download/getExcel/${res.data}`)
    downloadExcel(res.data)
